...
# --- 加载由新训练脚本生成的模型 ---
MODEL_PATH = os.path.join(SCRIPT_DIR, 'final_augmented_model.pth') 
...
if not os.path.exists(MODEL_PATH):
    print(f"错误: 增强版模型文件未找到！路径: '{MODEL_PATH}'")
    print("请先成功运行 train_augmented_unet.py 脚本。")
    exit()

model = smp.Unet(
    encoder_name=ENCODER,
    encoder_weights=PRETRAINED_WEIGHTS, # 权重将在下一步加载
    in_channels=3,
...
if not IMAGE_PATH or not os.path.exists(IMAGE_PATH):
    print(f"错误: 测试图片 '{IMAGE_PATH}' 未找到或路径无效。")
    exit()

print(f"正在评估图片: {IMAGE_PATH}")
...
# 3. 转换为Tensor (确保与训练时一致的数据类型)
preprocessed_image = preprocessed_image.astype('float32')
image_tensor = torch.from_numpy(preprocessed_image).permute(2, 0, 1).unsqueeze(0).to(DEVICE)
...
# 注意：这里我们用原始的、未缩放的图片进行叠加，以获得更清晰的对比
resized_original_for_overlay = cv2.resize(image, (pred_mask.shape[1], pred_mask.shape[0]))

comparison_image = np.hstack([resized_original_for_overlay, color_mask])

base_name = os.path.basename(IMAGE_PATH)
file_name_without_ext = os.path.splitext(base_name)[0]
output_filename = f"evaluation_augmented_{file_name_without_ext}.png"
